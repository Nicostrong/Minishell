...
/*   Updated: 22/09/2024 15:41:25 by Nicostrong       ###   ########.lu       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_fill_pipex(t_shell *shell)
{
	...
	while (current)
	{
		...
		{
			pipex->path_cmd = ft_concat_path_cmd(shell->pipex, current->value);
			pipex->a_cmd_opt[0] = ft_strdup(pipex->path_cmd);
		}
		else if (current->type == T_BUILTIN)
			pipex->builtin = 1;
		else if (current->type == T_OPT)
			pipex->a_cmd_opt = ft_append_str(pipex->a_cmd_opt, current->value);
		else if (current->type == T_F_OUT)
		{
			pipex->file_out = ft_strdup(current->next->value);
		...
		{
			pipex->file_in = ft_strdup(current->next->value);
			pipex->fd_in = open(pipex->file_in, O_WRONLY);
		}
		else if (current->type == T_HEREDOC)
		{
			pipex->here_doc = 1;
			pipex->limiter = ft_strdup(current->next->value);
		}
